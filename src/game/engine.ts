...
function resolveShowdown(room: Room): void {
  ...
  const primaryWinner = livePlayers[0];
  console.log('[Showdown] Winner:', primaryWinner?.id, primaryWinner?.name);
...
export function applyAction(room: Room, playerId: string, action: GameActionPayload): { room: Room; error?: string } {
  ...
  console.log('[Process] Action:', action.type, 'Amount:', action.amount);
  ...
  console.log('[Process] Active players:', activePlayers.length);
  console.log('[Process] Active IDs:', activePlayers.map((player) => player.id));

  const allInOnly = activePlayers.length > 1 && activePlayers.every((player) => player.status === 'allin');
  if (allInOnly) {
    while (room.status !== 'ended' && gameState.currentRound !== 'showdown') {
      advanceRound(room);
    }
    return { room };
  }
  ...
  if (gameState.activePlayerIds.length <= 1) {
    if (activePlayers.length === 1) {
      console.log('[Process] Only 1 left, should trigger showdown');
      console.log('[Process] Winner:', activePlayers[0]?.id);
    }
    gameState.currentRound = 'showdown';
    resolveShowdown(room);
    return { room };
